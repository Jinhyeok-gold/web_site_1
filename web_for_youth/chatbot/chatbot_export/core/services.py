...
def ask_expert_ai(user_message, user_data=None):
    ...
    def expert_fallback(msg, data):
        ...
        if any(x in msg_l for x in ['똑깥', '똑같', '반복', '이유', '누구', '뭐야', '에러']):
            reason = "현재 AI 가동을 위한 **API 키가 설정되지 않아** 기본 모드로 동작 중이라 답변이 정형화될 수 있어요. 😅"
            if not GOOGLE_API_KEY:
                return f"{reason}\n\n하지만 걱정 마세요! 제가 {name}님의 프로필을 바탕으로 계산한 **최적의 정책 리스트**는 변함없이 정확하답니다. 키를 `.env`에 등록하시면 더 다양한 대화가 가능해져요!"
            return "앗, 제가 너무 일관된 모습만 보여드렸나요? 더 구체적인 상황(소득, 거주지 등)을 말씀해 주시면 더 새로운 추천이 가능합니다!"
    ...
    try:
        ...
        # 현재 환경에서 가장 호환성이 높은 모델 사용
        model = genai.GenerativeModel('gemini-flash-latest')
        
        prompt = f"당신은 정책 전문가입니다. 다음 컨텍스트를 바탕으로 사용자에게 전문적이고 친절하게 답변하세요. 사용자 질문: '{user_message}', 프로필: {user_data}, 추천정책: {top_policies}. 정책 적합 지수를 근거로 설명해 주세요."
        response = model.generate_content(prompt)
        return response.text.strip()
    except Exception as e:
        print(f"AI ERROR (gemini-flash-latest): {str(e)}")
...
def generate_expert_report(user_data, top_matches):
    ...
    try:
        ...
        return "현재 분석 서버 점검 중입니다. 요약 리스트를 대신 확인해 주세요."
